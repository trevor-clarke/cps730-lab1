...
#.  Install nginx:
    On Ubuntu or Debian: :php:`sudo apt-get install nginx`
    On CentOS or Fedora: :php:`sudo yum install nginx`
    On macOS: :php:`brew install nginx`
    Once nginx is installed, you can start the nginx service using the following command:

#.  On Ubuntu or Debian: :php:`sudo systemctl start nginx`
    On CentOS or Fedora: :php:`sudo systemctl start nginx`
    On macOS: :php:`sudo nginx`
    To test if nginx is running, you can check the status of the nginx service using the following command:

#.  On Ubuntu or Debian: :php:`sudo systemctl status nginx`
    On CentOS or Fedora: :php:`sudo systemctl status nginx`
    On macOS: :php:`sudo nginx -t`
...
#.  After modifying the configuration file, you'll need to reload the nginx service to apply the changes. You can do this using the following command:
    On Ubuntu or Debian: :php:`sudo systemctl reload nginx`
    On CentOS or Fedora: :php:`sudo systemctl reload nginx`
    On macOS: :php:`sudo nginx -s reload`
...
To access the Nginx web server from outside the container, you need to expose the port of the container to the host system. This can be done by using the :kbd:`- p` flag when running the container.
